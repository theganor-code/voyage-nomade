...
async function galleryCompatibleBlob(file){
  ...
  if(typeof HeicTo!=="function")throw new Error("Le convertisseur des photos iPhone n'est pas chargé. Vérifie la connexion Internet puis réessaie.");
  const converted=await HeicTo({blob:file,type:"image/jpeg",quality:.92});
...
async function fileToDataURL(file){
  return galleryPhotoDataUrl(file);
}
